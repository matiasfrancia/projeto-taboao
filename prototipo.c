...
int playScreen(){
    ...
    while (!sair){
        ...
        if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP){
            if (evento.mouse.x >= 415 && evento.mouse.x <= 585 &&
        ...
        while (!al_is_event_queue_empty(fila_eventos)){
            ...
            int pom_eventos = al_wait_for_event_until(fila_eventos, &evento, &timeout);
     
            if (pom_eventos && evento.type == ALLEGRO_EVENT_DISPLAY_CLOSE){
                sair = 1;
            }
